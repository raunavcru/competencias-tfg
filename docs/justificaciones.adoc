...
|Alternativas
|Existen otras laternativas como:

* MySQL
* MariaDB

MySQL es la opción más extendida, pero su licencia puede ser un problema, ya que está sujeta a las desiciones de Oracle. La segunda alternativa es MariaDB, un fork de MySQL de código abierto. La gran diferencia de PostgreSQL con esta última, como hemos comentado anteriormente, es que pensada para alto volumen. Este ha sido el origen de nuestra elección.  
|===
